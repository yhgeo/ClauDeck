...
在 GUI 中点击 `安装自动同步` 后，ClauDeck 会把一个可移植的 Claude Code `SessionStart` hook 写入当前用户的 `~/.claude/settings.json`。之后 Claude Code 会话启动时会后台启动 watcher，切换模型或配置后也会自动补回缺失的 `enabledPlugins`。
