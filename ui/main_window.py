...
class PluginManagerWindow(QMainWindow):
    ...
    def _build_layout(self) -> None:
        ...
        subtitle = BodyLabel("管理 Claude Code 插件、同步启用状态，并浏览 README、Skills、Hooks、Agents 与 Commands。", central)
        title_column.addWidget(title)
        title_column.addWidget(subtitle)
        title_row.addLayout(title_column, 1)
        self.settings_button.setToolTip("同步设置：控制是否补齐新增插件，以及启用状态采用单向维护还是双向接受外部变化。")
